...
def _natural(s):
    s = str(s).strip()
    return [part.isdigit() and int(part) or part.lower() for \
        part in NUMS.split(s)]

def natural_sort(sortable, modifier=None):
    if modifier:
        sortable.sort(key = lambda x: _natural(modifier(x)))
    else:
        sortable.sort(key = lambda x: _natural(x))
...
def decode_heuristically(s, enc=None, denc=sys.getdefaultencoding()):
    ...
    try:
        ...
        encodings = ["utf-8","utf-16","iso-8859-1","cp1252","iso-8859-15"]
...
def decode_by_counting(s, enc=None, denc=sys.getdefaultencoding(),
    ...
    try:
        ...
        encodings = ["utf-8","iso-8859-1","cp1252","iso-8859-15", "utf-16"]
        # if the default encoding is not ascii it's a good thing to try
        if denc != "ascii": encodings.insert(0, denc)
        # always try any caller-provided encoding first
        if enc: encodings.insert(0, enc)
...
def to_unicode(text, charset=None):
    return decode_heuristically(text)[0]

__all__ = ['natural_sort', 'to_unicode']